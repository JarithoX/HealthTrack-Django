...
from professional_panel.models import ProfessionalComment
...
def index(request):
    ...
    # 4. Obtener último comentario del profesional
    ultimo_comentario = ProfessionalComment.objects.filter(patient_username=request.user.username).order_by('-created_at').first()
...
def mensajes_view(request):
    ...
    # Obtener todos los comentarios (conversación completa)
    ...
    ultimo_mensaje_pro = comentarios.filter(is_from_professional=True).last()
    professional_username = ultimo_mensaje_pro.professional if ultimo_mensaje_pro else None

    if request.method == 'POST':
        mensaje_texto = request.POST.get('mensaje')
        if mensaje_texto and professional_username:
            ProfessionalComment.objects.create(
                professional=professional_username,
                patient_username=username,
                comment=mensaje_texto,
